...
设置页可开启「启用本地验证模式」。开启后不调用桥、Codex 或 OpenAI，也不创建网络任务；仅支持「图像编辑」模式。插件会直接复制活动图层，并以当前选区创建该副本的图层蒙版，结果图层名称会以 `ComfyPSGPT - 本地验证` 开头，状态栏会显示选区外接矩形的尺寸和左上坐标。该操作不再导出、上传或生成 PNG，可快速确认活动图层、选区和图层蒙版回贴的 Photoshop 端流程。
