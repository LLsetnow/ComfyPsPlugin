...
def run_inpaint_blocking(image_path: Path, mask_path: Path, out_dir: Path) -> bytes:
    """阻塞流程:上传蒙版 → 跑工作流 → 读结果字节。丢到线程池执行。"""
    cfg = CONFIG
...
async def handle_run(request):
    ...
    try:
        ...
        result_bytes = await loop.run_in_executor(
            None, run_inpaint_blocking, img_path, mask_path, out_dir
        )
