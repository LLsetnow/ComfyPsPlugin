...
云扉（AIGate）创建 ComfyUI 实例时，不要为会频繁更新的镜像固定 `aigateCreate.imageId`：镜像重建后 `worksId` 会改变。应使用稳定镜像名动态解析，例如本项目的镜像别名为 `comfyps`：
...
- 每次创建实例时按 `imageName` 精确查询，个人镜像（`3`）优先、社区镜像（`2`）兜底，因此新镜像保持名称 `comfyps` 即可自动使用新的 ID。
- 同一时间只能保留一张同名 `comfyps` 镜像；存在同名旧镜像时，查询顺序无法保证会选中最新版本。
- 修改 `bridge/config.json` 后重启本地桥；该私有配置不提交到 Git。
...
- 云扉（AIGate）局部编辑固定使用 Boogu 变体；面板仍须显示 `wfResolution`，不能因后端是云扉而隐藏该控件。
- `setArgs` 将分辨率转成节点 `212` 的 `output_target_width` 和 `output_target_height`。`bridge.py` 必须把 `extraSetArgs` 传入 `run_native_inpaint`，再由 `build_native_workflow()` 用 `apply_set_args()` 应用到复制后的工作流。
- 不要只恢复前端控件或只修改 JSON 默认值：前端显示、`/run` 参数转发和原生工作流覆盖三段必须同时保留。返图仍会按 Photoshop 选区回贴，图层外观尺寸不一定等于推理分辨率。
- 相关回归覆盖放在 `plugin/test_aigate_native.js`、`bridge/test_aigate_bridge.py` 和 `bridge/test_aigate_native.py`。
